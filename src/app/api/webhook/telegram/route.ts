...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error && error.code === '23505') { // Code d'erreur PostgreSQL pour violation de contrainte unique
      await supabase
        .from('participants')
        .update({
          first_name: message.from.first_name,
      ...
      console.log(`Utilisateur existant mis à jour: ${message.from.first_name} (ID: ${message.from.id})`)
    } else if (!error) {
      console.log(`Nouveau participant ajouté: ${message.from.first_name} (ID: ${message.from.id})`)
    } else {
